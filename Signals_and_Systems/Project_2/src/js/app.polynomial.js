function seperateTerms(eq) {
	...
	for (var i = 1; i < eq.length; i++)
		if (mind > eq[i].delay) mind = eq[i].delay;
	mind = Math.abs(mind);
	for (var i = 0; i <= mind; i++) POLY_X.push(0), POLY_Y.push(0);
	for (var i = 0; i < eq.length; i++)
		if (eq[i].signal == CONST_X) POLY_X[mind + eq[i].delay] = eq[i].coef;
		else POLY_Y[mind + eq[i].delay] = eq[i].coef * -1;
	POLY_Y[mind] = 1;

	var polyxDeg = polyDegree(POLY_X),
		polyyDeg = polyDegree(POLY_Y),
		xmsb = POLY_X[polyxDeg],
		ymsb = POLY_Y[polyyDeg];
	var k = xmsb / ymsb;
	if (k != 0)
		for (var i = 0; i <= mind; i++) POLY_X[i] = POLY_X[i] / xmsb, POLY_Y[i] = POLY_Y[i] / ymsb;
	return {
		'k': k,
		'num': polyNormal(POLY_X, polyxDeg),
		'den': polyNormal(POLY_Y, polyyDeg),
	};
}
...
// remove redundent zero
function polyNormal(p, degree) {
	var np = [];
	for (var i = 0; i <= degree; i++) np.push(p[i]);
	return np;
}

function PolyReSolveT(rawArray) {
	...
	if (rawArrayLength < 3) {
		console.log("This utility requires AT LEAST three entries to be entered, but fewer than three entries have been detected. No further action taken.");
		return null;
	}
	...
	if (rawArrayLength > (MDP1 + 1)) {
		console.log("This utility accepts input of up to 102 entries: the first for the degree and up to 101 for the coefficients. However, more than 102 entries have been input. No further action taken.");
		return null;
	}
	...
	if (!isNaN(k)) { // Degree field contains a valid number; otherwise ignore
		...
		console.log("Invalid input for polynomial degree. No further action taken");
		return null;
	} // End else

	if (POLYDEGREE > MAXDEGREE) {
		console.log("This utility accepts polynomials of degree up to " + MAXDEGREE + ". However, a higher number has been input. No further action taken.");
		return null;
	}

	if (POLYDEGREE < 0) {
		console.log("Negative values for the polynomial degree are not accepted. No further action taken.");
		return null;
	}

	if (POLYDEGREE > (numCoeff - 1)) {
		console.log("There are too few coefficients entered for the polynomial degree input. Check the data. No further action taken.");
		return null;
	}
	...
	for (var i = 0; i < numCoeff; i++) { // Examine the data fields
		...
		if (!isNaN(tempx)) { // Field contains a valid number
			...
			console.log("Invalid input for entry " + (i + 1) + ". No further action taken");
			return null;
		} // End else
	} // End for i
	...
	if (k == 0) {
		console.log("Polynomial degree is 0. No further action taken.");
		return null;
	}

	if (POLYDEGREE != k) {
		console.log("The number of coefficients entered does not correspond with the polynomial degree input. Check the data. No further action taken.");
		return null;
	}
	POLYDEGREE = k;
	...
	var roots = [];
	for (var number = 0; number < POLYDEGREE; number++)
		roots.push(math.complex(zeror[number], zeroi[number]));
	return roots;
} //End of PolyReSolveT
